...
extern int     SetUp_Bar               =  1;                                                         // Setup Bar, 0 = Actual, 1 = Close  
...
int open_manual_mode(){
   ...
   if(ObjectFind(HL_BUY_OVER_Name) >= 0){
      value=ObjectGetValueByShift(HL_BUY_OVER_Name,SetUp_Bar);
      if(value>0.0 && Close[SetUp_Bar]>value){
   ...
   if(ObjectFind(HL_BUY_UNDER_Name) >= 0){
      value=ObjectGetValueByShift(HL_BUY_UNDER_Name,SetUp_Bar);
      if(value>0.0 && Close[SetUp_Bar]<value){
         if(current_spread()>NormalizeDouble(Max_Spread,2) && !Ignore_Max_Spread){
            Alert("SVEA "+Symbol()+", current spread is out of range, do not open manual open! Move Horizontal Line");
   ...
   if(ObjectFind(HL_SELL_OVER_Name) >= 0){
      value=ObjectGetValueByShift(HL_SELL_OVER_Name,SetUp_Bar);
      if(value>0.0 && Close[SetUp_Bar]>value){
         if(current_spread()>NormalizeDouble(Max_Spread,2) && !Ignore_Max_Spread){
            Alert("SVEA "+Symbol()+", current spread is out of range, do not open manual open! Move Horizontal Line");
   ...
   if(ObjectFind(HL_SELL_UNDER_Name) >= 0){
      value=ObjectGetValueByShift(HL_SELL_UNDER_Name,SetUp_Bar);
      if(value>0.0 && Close[SetUp_Bar]<value){
         if(current_spread()>NormalizeDouble(Max_Spread,2) && !Ignore_Max_Spread){
            Alert("SVEA "+Symbol()+", current spread is out of range, do not open manual open! Move Horizontal Line");
...
int close_manual(){
   ...
   if(ObjectFind(HL_CLOSE_OVER_Name) >= 0){
      ...
      value=ObjectGetValueByShift(HL_CLOSE_OVER_Name,SetUp_Bar);
      if(value>0.0 && Close[SetUp_Bar]>value){
